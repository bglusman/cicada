...
class ElixirParser:
            ...
            if target and arguments:
                ...
                if target_text == 'defmodule':
                    ...
                    if module_name and do_block:
                        ...
                        functions_with_specs = self._match_specs_to_functions(functions, specs)

                        # Extract aliases and function calls
                        aliases = self._extract_aliases(do_block, source_code)
                        function_calls = self._extract_function_calls(do_block, source_code)
            ...
            if operator and operand:
                ...
                for call_child in operand.children:
                    if call_child.type == 'identifier':
                        attr_name = source_code[call_child.start_byte:call_child.end_byte].decode('utf-8')
                        ...
                        if attr_name == 'moduledoc':
                            ...
                            for arg_child in operand.children:
                                if arg_child.type == 'arguments':
                                    doc_string = self._extract_string_from_arguments(arg_child, source_code)
                                    if doc_string:
                                        return doc_string
        ...
        for child in node.children:
            ...
            if child.type == 'call':
                ...
                for call_child in child.children:
                    if call_child.type == 'identifier':
                        target_text = source_code[call_child.start_byte:call_child.end_byte].decode('utf-8')
                        if target_text == 'defmodule':
                            is_defmodule = True
                            break
    ...
    def _extract_string_from_arguments(self, arguments_node, source_code: bytes) -> str | None:
            ...
            if child.type == 'string':
                ...
                for string_child in child.children:
                    if string_child.type == 'quoted_content':
                        content = source_code[string_child.start_byte:string_child.end_byte].decode('utf-8')
                        ...
                        if func_call:
                            ...
                            for fc_child in func_call.children:
                                if fc_child.type == 'identifier':
                                    func_name = source_code[fc_child.start_byte:fc_child.end_byte].decode('utf-8')
                                elif fc_child.type == 'arguments':
                                    param_types = self._extract_param_types(fc_child, source_code)
                ...
                for call_child in operand.children:
                    if call_child.type == 'identifier':
                        ...
                        if attr_name == 'doc':
                            ...
                            doc_info = self._parse_doc(operand, source_code, node.start_point[0] + 1)
                            if doc_info:
            ...
            if arg_child.type == 'call':
                ...
                for call_child in arg_child.children:
                    if call_child.type == 'identifier':
                        ...
                        arg_names = self._extract_argument_names(call_child, source_code)
                        arity = len(arg_names)
                break
            ...
            # Alias with tuple: alias MyApp.{User, Post}
                ...
                for dot_child in arg_child.children:
                    if dot_child.type == 'alias':
                        module_prefix = source_code[dot_child.start_byte:dot_child.end_byte].decode('utf-8')
                ...
                if module_prefix and tuple_node:
                    ...
                    for tuple_child in tuple_node.children:
                        if tuple_child.type == 'alias':
                            short_name = source_code[tuple_child.start_byte:tuple_child.end_byte].decode('utf-8')
                    ...
                    if kw_child.type == 'pair':
                        ...
                        for pair_child in kw_child.children:
                            if pair_child.type == 'keyword':
                                ...
                                key_text = source_code[pair_child.start_byte:pair_child.end_byte].decode('utf-8')
                            elif pair_child.type == 'alias':
                                alias_name = source_code[pair_child.start_byte:pair_child.end_byte].decode('utf-8')
                        ...
                        if key_text and 'as' in key_text and alias_name:
                            ...
                            for prev_arg in arguments_node.children:
                                if prev_arg.type == 'alias':
                                    full_name = source_code[prev_arg.start_byte:prev_arg.end_byte].decode('utf-8')
    ...
    def _parse_function_call(self, call_node, source_code: bytes) -> dict | None:
        ...
        for child in call_node.children:
            if child.type == 'dot':
                ...
                for dot_child in child.children:
                    if dot_child.type == 'alias':
                        module_name = source_code[dot_child.start_byte:dot_child.end_byte].decode('utf-8')
                    elif dot_child.type == 'identifier':
                        function_name = source_code[dot_child.start_byte:dot_child.end_byte].decode('utf-8')
        ...
        if function_name in ['alias', 'import', 'require', 'use', 'def', 'defp', 'defmodule',
