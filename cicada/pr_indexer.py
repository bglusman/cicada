...
class PRIndexer:
        ...
        try:
            ...
            result = subprocess.run(
                [
                    "gh", "pr", "list",
                    "--state", state,
                    "--json", "number,title,url,state,mergedAt,author,headRefOid",
                    "--limit", "10000"
                ],
                capture_output=True,
                text=True,
    ...
    def _fetch_pr_commits(self, pr_number: int) -> List[str]:
        ...
        try:
            result = subprocess.run(
                ["gh", "pr", "view", str(pr_number), "--json", "commits", "-q", ".commits[].oid"],
                capture_output=True,
                text=True,
                check=True,
                cwd=self.repo_path
    ...
    def merge_indexes(self, existing_index: Dict[str, Any], new_prs: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
    ...
    def index_repository(self, output_path: str = 'data/pr_index.json', incremental: bool = False):
        """
